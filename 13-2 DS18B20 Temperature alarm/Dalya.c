...
unsigned char OneWire_Init(void)
{
	...
	EA = 0; // 因为延时非常严格，关闭中断以防止读时序时因为中断而造成时序混乱
			// 但若中断关闭，又会影响其他模块，比如数码管的显示，秒表等功能，当然独立按键可以关闭，因为它对时效性要求不高
			// 这也是单总线的一个弊端，因为它对时序要求很高，所以优先级最高，不允许其他函数打断它
			// 所以可以选择一些集成模块来解决这个问题
			// 比如数码管的显示会影响，可以选择一些集成的数码管模块（专门带有芯片的那种），当它自己扫描自己即可避免它与单总线抢时间
			// 所以I2C其实要好一些，因为它有单独的时钟模块，即便打断它，也没事，因为打断了它，对应时钟也会停，下次进来再运行即可，不影响时序
